...
function createToolingLog(initialLogLevelName = 'silent') {
  ...
  class ToolingLog extends _stream.PassThrough {
    ...
    verbose(...args) {
      ...
      this.write(' %s ', (0, _chalk.magentaBright)('sill'), (0, _util.format)(...args));
    }

    debug(...args) {
      if (!logLevel.flags.debug) return;
      this.write(' %s ', (0, _chalk.dim)('debg'), (0, _util.format)(...args));
    }

    info(...args) {
      if (!logLevel.flags.info) return;
      this.write(' %s ', (0, _chalk.blue)('info'), (0, _util.format)(...args));
    }

    success(...args) {
      if (!logLevel.flags.info) return;
      this.write(' %s ', (0, _chalk.green)('succ'), (0, _util.format)(...args));
    }

    warning(...args) {
      if (!logLevel.flags.warning) return;
      this.write(' %s ', (0, _chalk.yellow)('warn'), (0, _util.format)(...args));
    }
    ...
    error(err) {
      ...
      this.write('%s ', (0, _chalk.red)('ERROR'), err.stack || err.message || err);
    }
